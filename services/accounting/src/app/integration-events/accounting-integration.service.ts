...
export interface IAccountingIntegrationEventService {
...
@Injectable()
export class AccountingIntegrationEventService implements IAccountingIntegrationEventService {
  constructor(
  ...
  getEventBusConfigByEventName(name: string): IQueueProducerConfig {
    switch (name) {
      case OrderPaymentSucceededIntegrationEvent.name:
        return {
          mode: ProducerMode.Exchange,
          exchange: this.configService.get<string>('accountingProducer.orderConfirmedPayment.exchange') || '',
          routingKey: this.configService.get<string>('accountingProducer.orderConfirmedPayment.routingKey') || '',
        };
      default:
...
export const AccountingIntegrationEventProvider = {
  provide: AccountingIntegrationEventService.name,
  useClass: AccountingIntegrationEventService,
};
